...
public class TimerEventsRule {
	...
	public void calculateEvents(@Fact(RulesAggregate.RULE_CALENDAR) final Calendar calendar,   @Fact(RulesAggregate.RULE_INPUT) DefaultRuleInput ruleInput) {
		...
		final String builder = TimerEventsBuilder.newBuilder().withMinEventTime(ruleInput.minEventTime()).with(ruleInput.isUpdateMode()).with(Key.T0, alarmTime).with(Key.T1,Collections.max(Arrays.asList(alarmTime,uptime)) ).with(Key.T6, Collections.max(Arrays.asList(downTime,ruleInput.minSunDownTime()))).build();
